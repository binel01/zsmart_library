...
def get_last_cdr(phone_number, date_time, cdr_type):
	...
	begin_time = root.xpath('//zsmart/Data/body/CDRDtoList/CDRDto/EventBeginTime')
	...
	acc_name_1 = root.xpath('//zsmart/Data/body/CDRDtoList/CDRDto/AcctResName1')
	
	response = [0]
	if (res_state == 'Successful'):
		print('The request has been executed successfully !')
		for i in range(0, len(event)):
			begin_time_2 = datetime.strptime(begin_time[i].text, '%Y-%m-%d %H:%M:%S')
			if begin_time_2 >= new_date:
				response = [event[i].text, begin_time[i].text, duration[i].text, charge_1[i].text, acc_name_1[i].text]
				break
	else:
